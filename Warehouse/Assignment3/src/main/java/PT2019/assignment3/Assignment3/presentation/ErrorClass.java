...
public class ErrorClass extends JFrame {
	...
	public ErrorClass(String errors) {
		...
		picLabel.add(goBack);
		error.setBounds(60, 20, error.getPreferredSize().width, error.getPreferredSize().height);
	...
	public void reset(int var) {
		...
		anotherView.setVisible(true);
			...
			OrderController ord = new OrderController(view, new OrdersBusiness(), new OrderDetailBusiness());
			view.setVisible(true);
